...
def SABR_market_vol(K, f, t_exp, alpha, beta, nu, rho):
    ...
    for i in range(0, len(K)):
        if K[i] == f:  # ATM equation in Managing smile risk
            ...
            output[i] = (alpha / f ** (1 - beta)) * (
                1 + (part_1 + part_2 + part_3) * t_exp
            )
            ...
            output[i] = (alpha * z * (1 + (part_1 + part_2 + part_3) * t_exp)) / (
...
def atm_sigma_to_alpha(f, t_exp, sigma_atm, beta, nu, rho):
    ...
    p_3 = -sigma_atm
    p_2 = (1 + (2 - 3 * rho**2) * nu**2 * t_exp / 24) / f ** (1.0 - beta)
...
def SABR_calibration(f, t_exp, sigma_atm, beta, strikes, vols, guess):
    ...
    popt, pcov = curve_fit(
        func_to_optimize, strikes, vols, p0=(guess[1], guess[2]), maxfev=10000
    )
